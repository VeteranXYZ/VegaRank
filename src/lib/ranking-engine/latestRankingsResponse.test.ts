...
import { buildLatestRankingsResponse } from "./latestRankingsResponse";

describe("latest rankings response", () => {
  ...
  it("allocates limited latest-rankings rows across populated major groups", () => {
    const response = buildLatestRankingsResponse({
      run: makeRun("run-4"),
      signals: [
        ...makeSignals({
...
function makeSignal(
  overrides: Partial<LatestRankingSignalRecord> & {
    id: string;
    scanRunId: string;
    symbol: string;
  },
...
function makeSignals({
  ...
  count: number;
  rankStart: number;
  signalLabel: LatestRankingSignalRecord["signalLabel"];
  actionBias: LatestRankingSignalRecord["actionBias"];
  primaryStructure: LatestRankingSignalRecord["primaryStructure"];
}) {

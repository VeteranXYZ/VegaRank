import type { ScannerReviewKey, ScannerReviewText } from "@/lib/shared/rankingTypes";
import {
  actionBiasByCode,
...
export const RANKING_RESULT_GROUPS = [
  "eligible",
  "watch",
  "overheated",
  "risk",
...
export type RankingResultGroup = (typeof RANKING_RESULT_GROUPS)[number];

export const RANKING_RESULT_REVIEW_TIERS = [
  "eligible",
  "watch_high",
  "watch_caution",
  "watch_low",
...
export type RankingResultReviewTier = (typeof RANKING_RESULT_REVIEW_TIERS)[number];

export type RankingResultCautionLevel = "none" | "caution" | "low";

export type RankingResultGroupInput = {
...
export type RankingResultReview = {
  reviewTier: RankingResultReviewTier;
  statusNoteKey: ScannerReviewKey;
  cautionLevel: RankingResultCautionLevel;
  statusReasonKeys: ScannerReviewText[];
};

export type RankingResultGroupSummary = Record<RankingResultGroup, number> & {
  totalSignals: number;
  confirmed: number;
  trend: number;
  watchSignals: number;
...
const DISPLAY_GROUP_ORDER: Record<RankingResultGroup, number> = {
  eligible: 0,
  watch: 1,
  overheated: 2,
  risk: 3,
...
const WATCH_REVIEW_TIER_ORDER: Partial<Record<RankingResultReviewTier, number>> = {
  watch_high: 0,
  watch_caution: 1,
  watch_low: 2,
};

export function classifyRankingResultGroup(
  signal: RankingResultGroupInput,
): RankingResultGroup {
...
function normalizeGroupCode(value: string | null | undefined): RankingResultGroup | "" {
  if (!value) {
    return "";
  }
  ...
  return RANKING_RESULT_GROUPS.includes(resultGroup as RankingResultGroup)
    ? (resultGroup as RankingResultGroup)
    : "";
}

export function getRankingResultReview(
  signal: RankingResultGroupInput & { resultGroup?: RankingResultGroup },
): RankingResultReview {
  const resultGroup = signal.resultGroup ?? classifyRankingResultGroup(signal);
  const rankScore = signal.rankScore ?? Number.NEGATIVE_INFINITY;
  const primaryStructure = normalizePrimaryStructure(signal.primaryStructure);
...
function getDetectedRiskCodes(signal: RankingResultGroupInput) {
  const detectedRiskTypes = signal.detectedRiskTypes ?? [];
...
export function compareRankingResultGroupItems<
  T extends RankingResultGroupInput & { resultGroup?: RankingResultGroup; symbol?: string },
>(left: T, right: T) {
  const leftGroup = left.resultGroup ?? classifyRankingResultGroup(left);
  const rightGroup = right.resultGroup ?? classifyRankingResultGroup(right);
...
function compareWatchItems<T extends RankingResultGroupInput & { symbol?: string }>(
  left: T,
  right: T,
) {
  const leftRank = left.rankScore ?? Number.NEGATIVE_INFINITY;
  const rightRank = right.rankScore ?? Number.NEGATIVE_INFINITY;
  const leftReview = getRankingResultReview({ ...left, resultGroup: "watch" });
  const rightReview = getRankingResultReview({ ...right, resultGroup: "watch" });
  const reviewDelta =
...
export function buildRankingResultGroups<T extends { resultGroup: RankingResultGroup }>(
  items: T[],
) {
  return {
...
export function summarizeRankingResultGroups(
  signals: Array<RankingResultGroupInput & { resultGroup?: RankingResultGroup }>,
): RankingResultGroupSummary {
  const summary: RankingResultGroupSummary = {

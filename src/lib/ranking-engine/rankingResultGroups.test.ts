...
describe("ranking result grouping", () => {
  ...
  it("groups eligible, watch, risk, and neutral signals", () => {
    ...
    expect(classifyRankingResultGroup({ signalLabel: "watch" })).toBe("watch");
    expect(classifyRankingResultGroup({ actionBias: "avoid" })).toBe("risk");
    expect(classifyRankingResultGroup({ actionBias: "ignore" })).toBe("neutral");
  });
  ...
  it("keeps detected-risk eligible rows in watch when the risk is not severe", () => {
    ...
    expect(classifyRankingResultGroup(signal)).toBe("watch");
    expect(getRankingResultReview(signal)).toMatchObject({
  ...
  it("keeps negative-rank watch rows deterministic and lower priority", () => {
    ...
    expect(classifyRankingResultGroup(negativeWatch)).toBe("watch");
    expect(getRankingResultReview(negativeWatch)).toMatchObject({

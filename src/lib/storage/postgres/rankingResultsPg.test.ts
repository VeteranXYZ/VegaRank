...
describe("PgRankingResultsStore latest ranking queries", () => {
  it("selects the latest successful run by run metadata", async () => {
    ...
    const run = await store.getLatestRankingRun({ timeframe: "4h" });
  ...
  it("keeps full-universe selection timeframe-specific for daily and weekly runs", async () => {
    ...
    const dailyRun = await store.getLatestRankingRun({
      timeframe: "1d",
      assetClass: "crypto",
      preferFullUniverse: true,
    });
    const weeklyRun = await store.getLatestRankingRun({
      timeframe: "1w",
      assetClass: "crypto",
      preferFullUniverse: true,
    });
  ...
  it("computes latest ranking candle coverage from filtered ranking rows only", async () => {
    ...
    await store.listLatestRankingSignalsForRun({
      scanRunId: "run-1",
      timeframe: "4h",
      assetClass: "crypto",
  ...
  it("can skip latest ranking coverage aggregation and use ranking history bars", async () => {
    const queries: string[] = [];
    const store = new PgRankingResultsStore(
      makePool((sql, params) => {
        queries.push(sql);

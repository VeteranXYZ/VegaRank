import type { HistoryVisualCheckData } from "./HistoryPageClient";

type VisualRun = HistoryVisualCheckData["snapshots"][number];
type VisualSnapshotRow =
  HistoryVisualCheckData["snapshotsByRunId"][string]["rows"][number];
type VisualObservationResponse =
  NonNullable<
    HistoryVisualCheckData["observationsByRunIdAndWindow"][string]
  >[1];
type VisualObservationRow = NonNullable<VisualObservationResponse>["rows"][number];
type VisualReadinessRun = NonNullable<
  HistoryVisualCheckData["readinessByRunId"][string]
>["selectedRun"];
...
export function buildHistoryVisualCheckData(): HistoryVisualCheckData {

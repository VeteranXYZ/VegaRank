...
import type { LatestRankingsGroupKey } from "./latestRankingsUi";
import {
  actionCodeByBias,
...
export function buildLatestRankingsPreviewResponse(): LatestRankingsResponse {
  const groups = {
    eligible: [
      makePreviewItem({
        id: "preview-btc",
  ...
  } satisfies Record<LatestRankingsGroupKey, LatestRankingItem[]>;
...
function makePreviewItem(overrides: PreviewItemInput): LatestRankingItem {
  ...
  return {
    ...
    metrics: {
      ...
      riskAdjustedScore: overrides.rankScore ?? null,
      setupQualityScore: overrides.opportunityScore ?? null,
      confidenceScore: overrides.confirmationScore ?? null,
      absoluteSetupScore: overrides.opportunityScore ?? null,
      ...
      riskPenalty: overrides.riskScore ?? null,
      qualityPenalty: null,
      volumeRank: null,
...
function toQualityCodes({
  ...
  qualityFlags?: string[];
  isLowQuality?: boolean;
}): LatestRankingItem["qualityCodes"] {
  ...
  ].filter((code): code is LatestRankingItem["qualityCodes"][number] =>
    Boolean(code),
  );
...
const qualityCodeByPreviewValue: Partial<
  Record<string, LatestRankingItem["qualityCodes"][number]>
> = {
  core: "QH_601",
  major: "QH_501",
  normal: "QH_001",

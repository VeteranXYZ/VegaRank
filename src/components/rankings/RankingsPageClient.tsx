...
import { RankingsFilters, type ScannerSortKey } from "./RankingsFilters";
import { RankingsTable } from "./RankingsTable";
import { SelectedRankingPanel } from "./SelectedRankingPanel";
import {
  isCachedSourceEnabledInUi,
  isLocalSourceEnabledInUi,
  SCANNER_BUILD_MARKER,
} from "./sourceUi";
import { scannerSignalOrder } from "@/lib/shared/rankingConfig";
...
export type RankingsFiltersState = {
  mode: ScannerMode;
...
export const initialRankingsFilters: RankingsFiltersState = {
  mode: "single",
  source: "remote",
  timeframe: "4h",
  mtfPreset: "short",
...
export function RankingsPageClient() {
  ...
  const [filters, setFilters] =
    useState<RankingsFiltersState>(initialRankingsFilters);
  ...
  function updateFilters(nextFilters: RankingsFiltersState) {
  ...
  return (
    <section className="mx-auto flex min-h-[calc(100vh-1px)] max-w-[1760px] flex-col bg-[var(--workspace-background)] px-2 py-2">
      ...
      <div className="grid min-h-0 flex-1 gap-2 xl:h-[calc(100vh-72px)] xl:grid-cols-[252px_minmax(0,1fr)_330px]">
        <RankingsFilters filters={filters} onChange={updateFilters} />
          ...
          <RankingsTable
            ...
            errorMessage={
              scanQuery.error instanceof Error
                ...
                : "Unable to load ranking results."
            }
        ...
        <SelectedRankingPanel result={selectedResult} />
      </div>
    </section>
  );
}
  ...
  if (!response.ok) {
    throw new Error(
      await getScannerErrorMessage({
        response,
        fallback: "Ranking request failed.",
        subrequestMessage:
...
export async function fetchBatchedSingleTimeframeScan(
  filters: RankingsFiltersState,
  options: FetchScanOptions,
) {
  const batchSize = 20;
  ...
  try {
    do {
      ...
      try {
        ...
        if (isAbortError(error) || responses.length === 0) {
          throw error;
        }
      ...
      if (!batchResponse.ok) {
        const message = await getScannerErrorMessage({
          response: batchResponse,
          fallback: "Ranking batch request failed.",
          subrequestMessage:
            "Cloudflare Free subrequest limit reached. Try a smaller batch size.",
        });
...
async function fetchMtfScan(
  ...
  if (!response.ok) {
    throw new Error(
      await getScannerErrorMessage({
        response,
        fallback: "MTF ranking request failed.",
        subrequestMessage:
...
export async function fetchBatchedMtfScan(
  filters: RankingsFiltersState,
  options: FetchScanOptions,
) {
  const batchSize = 8;
  ...
  try {
    do {
        ...
        return mergeBatchScanResponses(responses, [
          {
            ...
            message:
              error instanceof Error
                ? error.message
                : "MTF ranking batch request failed.",
            cursor,
          },
        ]);
      }
      ...
      if (!batchResponse.ok) {
        const message = await getScannerErrorMessage({
          response: batchResponse,
          fallback: "MTF ranking batch request failed.",
          subrequestMessage:
            "Cloudflare Free subrequest limit reached. Try a smaller MTF batch size.",
        });
...
export function shouldUseBatchedScan(filters: RankingsFiltersState) {
  return (
    filters.mode === "single" &&
...
export function shouldUseBatchedMtfScan(filters: RankingsFiltersState) {
  return (
    filters.mode === "mtf" &&
    filters.source === "remote" &&
    filters.maxSymbols === "ALL"
...
export function mergeBatchScanResponses(
  responses: ScanApiResponse[],
  batchWarnings: ScanApiResponse["batchWarnings"] = [],
) {
  if (responses.length === 0) {
    throw new Error("No ranking batches returned.");
  }
  ...
  return {
    ...
    batchWarnings: [
      ...responses.flatMap((response) => response.batchWarnings ?? []),
      ...batchWarnings,
    ],
    results,
...
function normalizeFilters(filters: RankingsFiltersState): RankingsFiltersState {

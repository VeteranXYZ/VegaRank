...
import type { MarketPhase, ScannerSignalState } from "@/lib/shared/rankingTypes";
import type { RankingsFiltersState } from "./RankingsPageClient";
...
type RankingsFiltersProps = {
  ...
  onChange: (filters: RankingsFiltersState) => void;
};
...
export function RankingsFilters({ filters, onChange }: RankingsFiltersProps) {
  const { dictionary: t } = useLanguage();
  const localSourceEnabled = isLocalSourceEnabledInUi();
  const cachedSourceEnabled = isCachedSourceEnabledInUi();
  const presets: Array<{ label: string; filters: RankingsFiltersState }> = [
    { label: t.scanner.resetView, filters: defaultRankingsFilters },
    {
      label: t.scanner.quickWatchlist,
      filters: {
        ...defaultRankingsFilters,
        mode: "single",
        timeframe: "4h",
        signal: "WATCHLIST",
  ...
  function update<K extends keyof RankingsFiltersState>(
    key: K,
      ...
      <div className="space-y-2.5 text-xs text-[var(--muted)]">
        <FilterSection title={t.scanner.sectionScan}>
          <label className="block">
            ...
            <select
              value={filters.mode}
              onChange={(event) =>
                update("mode", event.target.value as RankingsFiltersState["mode"])
              }
          ...
          <label className="block">
            ...
            <select
              ...
              onChange={(event) =>
                update("source", event.target.value as RankingsFiltersState["source"])
              }
          ...
          <label className="block">
            ...
            <select
              ...
              onChange={(event) =>
                update(
                  "timeframe",
                  event.target.value as RankingsFiltersState["timeframe"],
                )
              }
              ...
              onChange={(event) => {
                const nextValue =
                  event.target.value === "ALL"
                    ...
                    : (Number(
                        event.target.value,
                      ) as RankingsFiltersState["maxSymbols"]);
        ...
        <FilterSection title={t.scanner.sectionFilters}>
          ...
          <select
            ...
            onChange={(event) =>
              update("signal", event.target.value as RankingsFiltersState["signal"])
            }
              className={controlClass}
          >
          ...
          <select
            value={filters.phase}
            onChange={(event) =>
              update("phase", event.target.value as RankingsFiltersState["phase"])
            }
              className={controlClass}
          >
          ...
          <select
            ...
            onChange={(event) => {
              const nextLimit =
                event.target.value === "ALL"
                  ? "ALL"
                  : (Number(event.target.value) as RankingsFiltersState["limit"]);
...
const defaultRankingsFilters: RankingsFiltersState = {
  mode: "single",
  source: "remote",
  timeframe: "4h",
  mtfPreset: "short",

...
describe("ranking result filtering", () => {
  it("defaults the display count to 50 rows", () => {
    expect(initialRankingsFilters.limit).toBe(50);
  });

  it("defaults the live scan range to 100 symbols instead of ALL", () => {
    expect(initialRankingsFilters.maxSymbols).toBe(100);
  });

  it("keeps the default rankings source remote", () => {
    expect(initialRankingsFilters.source).toBe("remote");
  });
...
describe("ranking signal summary", () => {
...
describe("ranking batched fetch helpers", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });
  ...
  it("requests maxSymbols=100 for the default single-timeframe scan", async () => {
    ...
    await fetchSingleTimeframeScan(initialRankingsFilters, {});
...
function makeFilters(
  overrides: Partial<RankingsFiltersState> = {},
